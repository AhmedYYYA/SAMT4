(() => {
  ...
  const translations = {
    en: {
      ...
      "programme.story": "Discover the Story of SAMT",
    ...
    ar: {
      ...
      "programme.story": "اكتشف قصة سَمْت",
  ...
  function updateHeroDirection(animate = false) {
    const identity = q(".hero__identity");
    const copy = q(".hero__copy");
    const routes = q(".hero-routes");
    const rtl = currentLanguage === "ar";

    if (routes) {
      if (hasGSAP) gsap.set(routes, { scaleX: rtl ? -1 : 1, transformOrigin: "50% 50%" });
      else routes.style.transform = `scaleX(${rtl ? -1 : 1})`;
    }

    if (!animate || reducedMotion || !hasGSAP) return;
    if (copy) gsap.fromTo(copy, { x: rtl ? 34 : -34, autoAlpha: .35 }, { x: 0, autoAlpha: 1, duration: .72, ease: "power3.out", clearProps: "x,opacity,visibility" });
    if (identity) gsap.fromTo(identity, { x: rtl ? -70 : 70, autoAlpha: 0 }, { x: 0, autoAlpha: 1, duration: .9, ease: "power4.out", clearProps: "x,opacity,visibility" });
  }
  ...
  function runIntro() {
    ...
    tl.from(".site-header", { y: -32, autoAlpha: 0, duration: .8 })
      ...
      .from(".hero__identity", { x: currentLanguage === "ar" ? -64 : 64, autoAlpha: 0, duration: .95 }, "-=.7")
  ...
  function initMotion() {
    ...
    gsap.to(".hero-routes", { yPercent: 10, ease: "none", scrollTrigger: { trigger: ".hero", start: "top top", end: "bottom top", scrub: 1 } });
